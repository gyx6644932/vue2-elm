...
const actions = {
	...
	getAllData: async({
		...
		let friends = {}
		let weather = {}
		await that.$http.get('http://localhost:3000/api/self')
			.then(({
				data
			}) => {
				self = data.data
			})
		await that.$http.get('http://localhost:3000/api/friends')
			.then(({
				data
			}) => {
				friends = data.data
			})
		await that.$http.get('http://localhost:3000/api/weather')
			.then(({
				data
			}) => {
	...
	sendValue: async({
		...
		await that.$http.get('http://localhost:3000/api/robotapi', {
				params: {
					message,
					id: _id
				}
